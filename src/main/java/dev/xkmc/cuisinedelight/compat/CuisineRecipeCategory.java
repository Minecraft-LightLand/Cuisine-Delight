...
public class CuisineRecipeCategory extends BaseRecipeCategory<BaseCuisineRecipe<?>, CuisineRecipeCategory> {
	...
	public void setRecipe(IRecipeLayoutBuilder builder, BaseCuisineRecipe<?> recipe, IFocusGroup focuses) {
		for (int index = 0; index < 9; index++) {
			int x = index % 3;
			int y = index / 3;
			var r = builder.addSlot(RecipeIngredientRole.INPUT, x * 18 + 1, y * 18 + 1).setStandardSlotBackground();
			if (index < recipe.list.size()) {
				var e = recipe.list.get(index);
				r.addIngredients(e.ingredient()).addRichTooltipCallback((view, b) -> ingredientTooltip(e, view, b::add));
			}
		}
